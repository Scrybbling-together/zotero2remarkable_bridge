...
def test_missing_unread_folder_warns_and_fails(caplog):
    ...
    assert any("Failed to upload" in record.message for record in caplog.records if record.levelname == "ERROR")
...
def test_missing_read_folder_handled_gracefully(caplog):
    ...
    assert any("does not exist on reMarkable" in record.message for record in caplog.records if record.levelname == "INFO")
...
def test_remarkable_api_unavailable_preserves_tags(caplog):
    ...
    mock_zotero = MockZoteroAPI()
    mock_rm = MockReMarkableAPI(files={}, folders={"", "Zotero", "Zotero/unread", "Zotero/read"}).api_unavailable()
    folders = {"unread": "unread", "read": "read"}
    ...
    assert any("reMarkable API is unavailable" in record.message for record in caplog.records if record.levelname == "ERROR")
    assert any("Failed to upload" in record.message for record in caplog.records if record.levelname == "ERROR")
