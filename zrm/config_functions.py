...
def load_config(config_file):
    ...
    zot = zotero.Zotero(
        config_dict["LIBRARY_ID"], config_dict["LIBRARY_TYPE"], config_dict["API_KEY"]
    )
    folders = {
        "unread": normalize_rm_path(config_dict["UNREAD_FOLDER"]),
        "read": normalize_rm_path(config_dict["READ_FOLDER"]),
    }
...
def write_config(file_name):
    ...
    print(
        "On your ReMarkable you should have created a folder called Zotero in the root directory.\nIn the following specify ONLY the names of the subfolders, e. g 'read' instead of 'zotero/read'.\n"
    )
    config_data["UNREAD_FOLDER"] = input(
        "Which ReMarkable folder should files be synced to? "
    )
    config_data["READ_FOLDER"] = input(
        "Which ReMarkable folder should files be synced from? "
    )
    print("You can find your library ID on this page")
    print(
        'Under Applications > user ID, "Your user ID for use in API calls is {COPY THIS}"'
    )
    ...
    config_data["USE_WEBDAV"] = input(
        "Does Zotero use WebDAV storage for file sync (True/False)? "
    )
    if config_data["USE_WEBDAV"].lower() == "true":
        config_data["WEBDAV_HOSTNAME"] = input(
            "Enter path to WebDAV folder (same as in Zotero config): "
        )
        config_data["WEBDAV_USER"] = input("Enter WebDAV username: ")
        config_data["WEBDAV_PWD"] = input(
            "Enter WebDAV password (consider creating an app token as password is safed in clear text): "
        )
...
def normalize_rm_path(folder_name: str) -> str:
    ...
    match folder_name:
        case str(s) if s.startswith("/") or s.startswith(" "):
            return normalize_rm_path(
                s[1:]
            )  # recursively removes all leading slashes and whitespaces
